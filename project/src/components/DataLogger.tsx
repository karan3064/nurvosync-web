...
export default function DataLogger({ leftPressure, rightPressure, leftImu, rightImu }: DataLoggerProps) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 p-4 rounded-xl text-white max-w-sm">
      <h3 className="text-sm font-bold text-gray-400 mb-3 uppercase tracking-wider">ML Training Data Collection</h3>
      ...
      <div className="mb-4">
        <label className="text-xs text-gray-400 block mb-1">Target Activity (Label)</label>
        <select
          value={activityLabel}
          onChange={(e) => setActivityLabel(e.target.value)}
          disabled={isRecording}
          className="w-full bg-black border border-gray-700 text-white p-2 rounded focus:border-blue-500 outline-none"
        >
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={handleClear}
          disabled={isRecording}
          className="bg-gray-800 hover:bg-gray-700 p-2 rounded disabled:opacity-50 transition-colors"
          title="Clear Buffer"
        >
          <Trash2 size={16} className={isRecording ? "text-gray-600" : "text-gray-400"} />
        </button>
      </div>

      <div className="mt-3 text-xs font-mono text-gray-500 flex justify-between">
        <span>Status: <span className={isRecording ? "text-red-400" : "text-gray-400"}>{isRecording ? "RECORDING" : "STANDBY"}</span></span>
